...
(function() {
  ...
  function buildDisplayText(song, artist) {
    ...
    return cleanSong && cleanArtist ? `${cleanSong} - ${cleanArtist}` : (cleanSong || cleanArtist || 'ATIVIDADE FM 103.1FM');
  }
  ...
  function handleIncomingStreamTitle(title) {
  ...
  function closeMetadataSubscription() {
    if (state.metadataEventSource) {
      try { state.metadataEventSource.close(); } catch (e) {}
      state.metadataEventSource = null;
    }
    state.metadataSourceUrl = '';
    state.metadataSourceKind = '';
  }

  function getActiveMetadataSource() {
    const config = window.__atividadeCurrentStreamConfig || {};
    const mode = state.specialMode || config.mode || 'zeno';

    if (mode === 'voz') {
      return { kind: 'voz', url: '' };
    }

    if (config.metadataeventsourceurl) {
      return {
        kind: mode === 'fallback' ? 'fallback' : 'zeno',
        url: config.metadataeventsourceurl
      };
    }

    return {
      kind: 'zeno',
      url: 'https://api.zeno.fm/mounts/metadata/subscribe/z2h3tpp2fchvv'
    };
  }

  function subscribeToMetadata(forceRestart) {
    const source = getActiveMetadataSource();

    if (!source.url) {
      closeMetadataSubscription();
      return;
    }

    if (!forceRestart && state.metadataEventSource && state.metadataSourceUrl === source.url && state.metadataSourceKind === source.kind) {
      return;
    }
    ...
    try {
      ...
      state.metadataEventSource = es;
      state.metadataSourceUrl = source.url;
      state.metadataSourceKind = source.kind;

      es.onmessage = function(event) {
        try {
          const parsed = JSON.parse(event.data);
          if (parsed && parsed.streamTitle) {
            handleIncomingStreamTitle(parsed.streamTitle);
          }
        } catch (e) {}
      };
