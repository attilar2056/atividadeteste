...
(function() {
  ...
  function splitRawMetadataTitle(rawTitle) {
    ...
    if (!cleaned) return { raw: '', first: '', second: '', hasSeparator: false };

    const separatorMatch = cleaned.match(/(.+?)\s-\s(.+)/);
    if (separatorMatch) {
      return {
        raw: cleaned,
        first: cleanupMetadataChunk(separatorMatch[1]),
        second: cleanupMetadataChunk(separatorMatch[2]),
        hasSeparator: true
      };
    }
  ...
  function parseMetadataForDisplay(rawTitle) {
    ...
    const guess = guessArtistSong(pieces.first, pieces.second);
    return {
  ...
  function resolveTrackMetadata(rawTitle, callback) {
    ...
    const fullQuery = cleanTitleForSearch(first, second) || cleanupMetadataChunk(rawTitle);
